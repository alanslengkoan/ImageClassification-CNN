...
EPOCHS           = 50           # Stage 2: fine-tune top layers
LR_HEAD          = 5e-4         # LR Stage 1 (head warmup)
LEARNING_RATE    = 2e-4         # LR Stage 2 (fine-tune)
NUM_CLASSES      = 3            # 3 kelas: baik, sedang, berat
FINE_TUNE_LAYERS = 20           # Jumlah layer backbone yang di-unfreeze di Stage 2
...
x       = base_model(inputs, training=False)   # BN pakai stats pre-trained (Stage 1)
...
# ============================================================
print('\n' + '=' * 60)
print('🚀 TRAINING ResNet50 — TWO-STAGE (LINUX)')
print(f'   📌 Strategi  : Gabung ringan+sedang → sedang')
print(f'   Stage 1      : {WARMUP_EPOCHS} epoch, backbone frozen, LR={LR_HEAD}')
print(f'   Stage 2      : maks {EPOCHS} epoch, top {FINE_TUNE_LAYERS} layers, LR={LEARNING_RATE}')
...
frozen_s2    = sum([1 for l in base_model.layers if not l.trainable])
print(f'   Frozen           : {frozen_s2} layer')
print(f'   Trainable        : {trainable_s2} layer ✅')
...
print(f'\n🔥 STAGE 2: Fine-tune top {FINE_TUNE_LAYERS} layers...')
history2 = model.fit(
    train_generator,
    epochs=EPOCHS,
    validation_data=val_generator,
...
print(f'   Epoch Stage 1     : {WARMUP_EPOCHS}')
print(f'   Epoch Stage 2     : {len(history2.history["accuracy"])}')
...
fig.suptitle('ResNet50 Fine-Tuning — Klasifikasi Kerusakan Jalan (3 KELAS)\n'
             f'(Two-Stage · Stage1={WARMUP_EPOCHS}ep LR={LR_HEAD} · Stage2={EPOCHS}ep LR={LEARNING_RATE})',
...
ax1.axvline(x=WARMUP_EPOCHS - 0.5, color='gray', linewidth=1.5, linestyle=':', label='Stage 2 start')
...
ax2.axvline(x=WARMUP_EPOCHS - 0.5, color='gray', linewidth=1.5, linestyle=':')
...
# ============================================================
print('\n' + '=' * 60)
print('📋 RINGKASAN HASIL — ResNet50 Two-Stage Fine-Tuning 3 KELAS')
print('=' * 60)
print(f'   Backbone          : ResNet50 pretrained ImageNet')
print(f'   📌 Strategi       : Gabung ringan+sedang → sedang')
print(f'   Kelas             : {NUM_CLASSES} kelas → {CLASS_LABELS}')
print(f'   Stage 1           : {WARMUP_EPOCHS} epoch, frozen backbone, LR={LR_HEAD}')
print(f'   Stage 2           : {len(history2.history["accuracy"])} epoch, top {FINE_TUNE_LAYERS} layers, LR={LEARNING_RATE}')
print(f'   Total epoch       : {total_epochs}')
print(f'   Class Weight      : ✅ Aktif')
